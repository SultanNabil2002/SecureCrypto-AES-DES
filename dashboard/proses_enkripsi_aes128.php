<?php
// File: Kriptografi-Aes/dashboard/proses_enkripsi_aes128.php
...
if (!$conn) {
    echo "<script>alert('ENKRIPSI AES GAGAL! Tidak bisa terhubung ke database.'); window.location.href='enkripsi_aes128.php';</script>";
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['encrypt_now'])) {
    ...
    $kunci_aes_128_bit = substr(md5($_POST["pwdfile"]), 0, 16); // Kunci 16 byte untuk AES-128
    $deskripsi_file = isset($_POST['desc']) ? mysqli_real_escape_string($conn, $_POST['desc']) : '';
    $algoritma_yang_digunakan = "AES-128";
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] == UPLOAD_ERR_OK) {
        ...
        $nama_file_enkripsi_unik = time() . "_" . uniqid() . "_AES_" . $nama_file_tanpa_ekstensi_aman . "." . $ekstensi_file . ".enc";
        ...
        if (!in_array($ekstensi_file, $ekstensi_diizinkan)) {
            echo ("<script>window.alert('Format file tidak diizinkan.'); window.location.href='enkripsi_aes128.php';</script>");
            exit();
        }

        // Validasi ukuran file
        if ($ukuran_file_kb_asli > 8192) { // Batas 8MB
            echo ("<script>window.alert('Ukuran file tidak boleh lebih besar dari 8MB.'); window.location.href='enkripsi_aes128.php';</script>");
            exit();
        }
...
// ...
        ...
        // Menyiapkan query INSERT awal ke database
        ...
        if (!$stmt_insert) {
             echo "<script>
                ...
                window.location.href='enkripsi_aes128.php';
            </script>";
            exit();
        }
        ...
        mysqli_stmt_bind_param($stmt_insert, "ssssdssss", 
            ...
            $ukuran_file_kb_asli,
            $kunci_aes_128_bit, // Menggunakan kunci AES 16 byte
            $algoritma_yang_digunakan, // "AES-128"
            $status_awal,
            $deskripsi_file
        );
        ...
        if (mysqli_stmt_execute($stmt_insert)) {
            ...
            // ... (lanjutan di Part 3: Proses Enkripsi Inti dengan Magic Number) ...
...
// ...
            ...
            if ($file_hasil_enkripsi) {
                ...
                // 1. Tambahkan "Magic Number" sebagai tanda pengenal file AES dari sistem ini
                ...
                for ($i = 0; $i < $total_bytes; $i += $block_size) {
                    ...
                    // Panggil metode enkripsi dari kelas AES
                    $ciphertext_final .= $aes->encryptBlock($chunk);
                }
...
// ...
            ...
            if ($berhasil_sepenuhnya && (filesize($path_hasil_enkripsi_absolut) > 0 || filesize($file_tmp_path) == 0) ) {
                ...
                if ($stmt_update) {
                    ...
                    // Tampilkan alert sukses (teks "dengan Magic Number" dihapus sesuai permintaan)
                    echo "<script>
            ...
            } else { // Jika $berhasil_sepenuhnya false atau file output kosong padahal input tidak
...
// Tutup kurung kurawal untuk blok if (isset($_FILES['file']) ...)
    ...
    echo ("<script>
        window.alert('ENKRIPSI " . ($algoritma_yang_digunakan ?? 'AES') . " GAGAL!\\n" . addslashes($error_msg) . "');
        window.location.href='enkripsi_aes128.php';
    </script>");
    exit();
}
